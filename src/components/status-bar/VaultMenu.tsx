...
function buildVaultActions(
  t: (key: string) => string,
  handlers: Pick<VaultMenuProps, 'onCreateEmptyVault' | 'onCloneGettingStarted' | 'onCloneVault' | 'onOpenLocalFolder'>,
): VaultAction[] {
  const { onCreateEmptyVault, onCloneGettingStarted, onCloneVault, onOpenLocalFolder } = handlers
  ...
  if (onCreateEmptyVault) {
    items.push({
      key: 'create-empty',
      icon: <Plus size={12} />,
      label: t('status_bar.create_empty_vault'),
  ...
  if (onOpenLocalFolder) {
    items.push({
      ...
      label: t('status_bar.open_local_folder'),
  ...
  if (onCloneVault) {
    items.push({
      ...
      label: t('status_bar.clone_git_repo'),
  ...
  if (onCloneGettingStarted) {
    items.push({
      ...
      icon: <Rocket size={12} />,
      label: t('status_bar.clone_getting_started'),
...
function VaultMenuItem({ vault, isActive, canRemove, onSelect, onRemove }: VaultMenuItemProps) {
  ...
  const removeLabel = t('status_bar.remove_vault', { name: vault.label })
  const itemClassName = [
  ...
  return (
    <div className="group relative flex w-full items-center rounded-sm">
      <Button
        ...
        title={unavailable ? t('status_bar.vault_not_found', { path: vault.path }) : vault.path}
...
export function VaultMenu({
  ...
  const actions = useMemo<VaultAction[]>(() => {
    return buildVaultActions(t, {
      onCreateEmptyVault,
      onCloneGettingStarted,
      onCloneVault,
      onOpenLocalFolder,
    })
  }, [t, onCreateEmptyVault, onCloneGettingStarted, onCloneVault, onOpenLocalFolder])

  return (
    <div ref={menuRef} style={{ position: 'relative' }}>
      <ActionTooltip copy={{ label: t('status_bar.switch_vault') }} side="top">
        <Button
          type="button"
          variant="ghost"
          size="xs"
          ...
          aria-label={t('status_bar.switch_vault')}
          data-testid="status-vault-trigger"
        >
          <FolderOpen size={13} />
          {activeVault?.label ?? t('status_bar.vault')}
        </Button>
      </ActionTooltip>
      {open && (
        <div
